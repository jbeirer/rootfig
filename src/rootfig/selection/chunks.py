        ...
        if second is None or THREADS == 1:
            ...
            try:
                for chunk in chain([first, second], iterator):
                    # in a copy of the caller's context, so that its np.errstate (and, from
                    # Python 3.14, its warning filters) applies as it would in the caller
                    pending.append(pool.submit(contextvars.copy_context().run, run, chunk))
