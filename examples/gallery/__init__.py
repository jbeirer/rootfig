...
``examples/out/`` in a few seconds; ``--style CMS`` draws them in another of the
:data:`STYLES`. The same functions feed the gallery pages of the documentation
(code shown next to each figure) and ``tests/test_gallery.py`` (pixel comparison
against ``docs/images/gallery/``). The tests render an example that takes a
``style`` in every entry of :data:`STYLES`, each as shown and inside
``rf.dark_theme()`` for dark pages, so the pictures in the documentation are made
by the code shown.

This module holds what a reader of the gallery is looking for: the shared
:func:`define` block, the styles and the examples, section by section. The toy
data lives in :mod:`.data`, the registry and the source extraction in
:mod:`.registry`.
"""
...
from .registry import DEFAULT_STYLE, EXAMPLES, Example, body_source, example, style_source
...
STYLES: dict[str, rf.Style] = {
    DEFAULT_STYLE: rf.Style(),
    ...
    "LHCb": rf.Style(experiment="LHCb", status="Preliminary", lumi=9, com=13.6),
    "ALICE": rf.Style(experiment="ALICE", status="Preliminary"),
    "DUNE": rf.Style(experiment="DUNE", status="Preliminary"),
}
"""The styles the gallery shows every example in that takes a ``style``: the neutral
default and one for each experiment with an mplhep style sheet and label."""


def styles_of(ex: Example) -> dict[str, rf.Style]:
    """The styles ``ex`` is drawn in: all of :data:`STYLES`, or the default alone."""
    return STYLES if ex.styled else {DEFAULT_STYLE: STYLES[DEFAULT_STYLE]}
...
def define() -> Dataset:
    """Describe the samples and variables that more than one example needs.

    The body of this function is the *Setup* block of the gallery pages, so it
...
# --------------------------------------------------------------------------------------
# Examples. Parameters are Dataset attributes, plus ``style`` for an example shown in every
# style; bodies are what a user would write in the directory holding the toy files.
# --------------------------------------------------------------------------------------

GETTING_STARTED = "Getting started"
SIMULATION_AND_DATA = "Simulation and data"
BINNING = "Binning and ranges"
SELECTIONS = "Selections and expressions"
BEYOND_1D = "Beyond one dimension"
APPEARANCE = "Appearance"


@example("quick", "The one-liner", section=GETTING_STARTED)
def quick(style: rf.Style) -> rf.Plot:
...
@example(
    "overlay_ratio", "Several samples, normalised, with a ratio panel", section=GETTING_STARTED
)
...
@example("arrays", "In-memory arrays instead of files", section=GETTING_STARTED, toy_data=False)
def arrays(style: rf.Style) -> rf.Plot:
...
@example("many_plots", "Many plots in a loop, saved by variable name", section=GETTING_STARTED)
def many_plots(mc: list[rf.Sample], data: rf.Sample) -> rf.Plot:
...
@example(
    "stack_data", "Stacked simulation with data and a ratio panel", section=SIMULATION_AND_DATA
)
...
@example(
    "xbreak_ratio",
    "A broken x axis: peak and far tail without the empty middle",
    section=SIMULATION_AND_DATA,
)
def xbreak_ratio(
...
def variable_bins(mc: list[rf.Sample], data: rf.Sample, style: rf.Style) -> rf.Plot:
...
@example("log_axes", "Logarithmic axes with log-spaced bins", section=BINNING)
def log_axes(signal: rf.Sample, zjets: rf.Sample, diboson: rf.Sample, style: rf.Style) -> rf.Plot:
...
@example("density_flow", "Density normalisation and summed overflow", section=BINNING)
def density_flow(
    signal: rf.Sample, zjets: rf.Sample, diboson: rf.Sample, style: rf.Style
) -> rf.Plot:
    """``normalize="density"`` makes the integral one and ``flow="sum"`` adds the under- and
    overflow to the edge bins, which is why the last bin sticks out."""
    return rf.plot(
        [zjets, diboson, signal],
...
@example("object_vs_event", "Per-object versus per-event selections", section=SELECTIONS)
...
@example("expressions", "Expressions and pre-filled histograms", section=SELECTIONS)
...
@example("hist2d", "A two-dimensional histogram", section=BEYOND_1D)
def hist2d(signal: rf.Sample, met: rf.Variable, style: rf.Style) -> rf.Plot:
...
@example("correlation", "A correlation matrix", section=BEYOND_1D)
def correlation(signal: rf.Sample, style: rf.Style) -> rf.Plot:
...
def profile(signal: rf.Sample, zjets: rf.Sample, met: rf.Variable, style: rf.Style) -> rf.Plot:
...
@example("fill_stats", "Filled histograms, a statistics box and free text", section=APPEARANCE)
def fill_stats(signal: rf.Sample, zjets: rf.Sample, met: rf.Variable, style: rf.Style) -> rf.Plot:
...
def main(out: Path = DEFAULT_OUT, style: str = DEFAULT_STYLE) -> None:
    """Write the toy dataset and every example figure into ``out``.

    An example that takes a style is drawn in ``STYLES[style]``.
    """
    ...
    for number, ex in enumerate(EXAMPLES, start=1):
        plot = ex.run(dataset, cwd=out, style=STYLES[style])
